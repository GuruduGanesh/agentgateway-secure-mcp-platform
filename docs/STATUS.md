...
| 2026-06-25 | Enterprise structure/process layer (mirrors upstream repo) | ✅ done | Added `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `DEVELOPMENT.md`, `CHANGELOG.md`, `.github/` (CODEOWNERS, dependabot, issue/PR templates, `validate.yml` static CI), `Makefile` + `tasks.ps1` task runner, `tests/validate.ps1`, `.gitattributes`, `.dockerignore`, `docs/README.md` index. README updated with repeatable-setup, docs map, contributing, refreshed folder tree. Validated locally via `tests/validate.ps1`. |
